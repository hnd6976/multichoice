...
const VerifyCode = ({ route, navigation }) => {
 ...
 const verifyEmail = async()=>{
  ...
  try{
    ...
    const response = await axios.post(`${API_URL}/auth/verifyEmail?email=${email}&token=${value}`)
    console.log(response.data.message);
    if(response.data.message=="success"){
      showConfirmDialog(navigation,"Xác nhận thành công")
    //Alert.alert("","Mã xác nhận mới đã được gửi")
  }
  else{
    showConfirmDialog(navigation,"Tài khoản đã được kích hoạt trước đó")
  }
/*
