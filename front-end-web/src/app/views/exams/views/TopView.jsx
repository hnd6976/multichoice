...
export default function TopCompleted() {
  ...
  const [topCompletedList, setTopCompletedList] = useState([]);
  const getTopCompletedAll = async () => {
    const response = await axios.get(
      `${process.env.REACT_APP_API_URL}/admin/dashboard/exam/getTopCompletedAll`
    );
    const list = response.data;
    console.log(list);
    let listTopCompleted = [];
    list.map((item, index) => {
      let it = { name: '', value: 0, avg: '' };
      it.name = item[0];
      it.value = item[1];
      it.avg = ((item[2] / item[3].questions.length) * 100).toFixed(2);
      listTopCompleted[index] = it;
    });
    setTopCompletedList(listTopCompleted);
  };
  useEffect(() => {
    getTopCompletedAll();
  }, []);
  const getTopCompletedYear = async () => {
    const response = await axios.get(
      `${process.env.REACT_APP_API_URL}/admin/dashboard/exam/getTopCompletedYear/${year}`
    );
    const list = response.data;
    let listTopCompleted = [];
    list.map((item, index) => {
  ...
  const getTopCompletedMonth = async () => {
    const response = await axios.get(
      `${process.env.REACT_APP_API_URL}/admin/dashboard/exam/getTopCompletedMonth/${year}/${month}`
    );
    const list = response.data;
    let listTopCompleted = [];
    list.map((item, index) => {
  ...
  const getTopCompletedDay = async () => {
    const response = await axios.get(
      `${process.env.REACT_APP_API_URL}/admin/dashboard/exam/getTopCompletedDay/${year}/${month}/${day}`
    );
    const list = response.data;
    let listTopCompleted = [];
    list.map((item, index) => {
  ...
  return (
    <Box sx={{ padding: 2, display: 'flex' }}>
      <Box sx={{}}>
        <Typography sx={{ fontSize: 18, fontWeight: 'bold' }}>Top 5 lượt làm bài</Typography>
            ...
            <Table sx={{ width: 450 }} aria-label="simple table">
              <TableHead>
                <TableRow>
                  ...
                  <TableCell align="center">Lượt làm bài</TableCell>
                  <TableCell align="center">Tỉ lệ đúng</TableCell>
                </TableRow>
              </TableHead>
              <TableBody>
                {topCompletedList.map((row, index) => (
                  <TableRow
                    key={row.name}
