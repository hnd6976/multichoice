...
const AddExam = ({}) => {
  ...
  const onSubmit = async () => {
    ...
    try {
      const res = await axios.post(`${process.env.REACT_APP_API_URL}/admin/exam/add`, formData);
      setIsLoading(false);
  ...
  useEffect(() => {
    setIsValidData(isValidTittle && isValidGrade && isValidSubject && isVaLidNum);
  }, [isValidTittle, isValidGrade, isValidSubject, isVaLidNum]);
  useEffect(() => {
    setIsVaLidNum(choosenQuestion.filter((e) => e != null).length == num);
  });
  useEffect(() => {
    if (examData.title.length >= 4 && examData.title.length <= 50) {
        ...
        <Filter sx={{ display: 'flex' }}>
          ...
          >
            ...
            {!isValidTittle && isValidTittle != null && (
              <Typography color={'red'}>Tiêu đề phải trên 4 và ít hơn 50 ký tự !</Typography>
            )}
          </Box>
          <TextField
