...
export default function ViewCharts() {
  ...
  const getListYear = async () => {
    const response = await axios.get(
      `${process.env.REACT_APP_API_URL}/admin/dashboard/user/getListYear`
    );
    console.log(response.data);
    const list = response.data;
    let listTemp = [];
    list.map((item, index) => {
      let it = { id: index, name: item, value: item };
      listTemp[index] = it;
    });
  ...
  const getListMonth = async () => {
    const response = await axios.get(
      `${process.env.REACT_APP_API_URL}/admin/dashboard/user/getListMonth/${year}`
    );
    console.log(response.data);
    const list = response.data;
    let listTemp = [];
  ...
  const getListDay = async () => {
    const response = await axios.get(
      `${process.env.REACT_APP_API_URL}/admin/dashboard/user/getListDay/${year}/${month}`
    );
    console.log(response.data);
    const list = response.data;
    let listTemp = [];
  ...
  const getCountCreatedEachYear = async () => {
    setDefault();
    const response = await axios.get(
      `${process.env.REACT_APP_API_URL}/admin/dashboard/user/getCountCreatedEachYear`
    );
    console.log(response.data);
    const list = response.data;
    let xAxisList = [];
  ...
  const getCountCreatedEachMonth = async () => {
    setDefault();
    const response = await axios.get(
      `${process.env.REACT_APP_API_URL}/admin/dashboard/user/getCountCreatedEachMonth/${year}`
    );
    console.log(response.data);
    const list = response.data;
    let xAxisList = [];
  ...
  const getCountCreatedEachDay = async () => {
    setDefault();
    const response = await axios.get(
      `${process.env.REACT_APP_API_URL}/admin/dashboard/user/getCountCreatedEachDay/${year}/${month}`
    );
    console.log(response.data);
    const list = response.data;
    let xAxisList = [];
  ...
  useEffect(() => {
    getCountCreatedEachMonth();
  }, []);
  useEffect(() => {
    if (filter == 'year') {
      getCountCreatedEachYear();
    }
    if (filter == 'month') {
      getCountCreatedEachMonth();
    }
    if (filter == 'day') {
      getCountCreatedEachDay();
    }
  }, [filter, year, month]);
  ...
  return (
    <Box sx={{ padding: 3, display: 'flex' }}>
      <Box sx={{}}>
        <Typography sx={{ fontSize: 18, fontWeight: 'bold' }}>Tạo mới</Typography>
        ...
        {false && xAxis.length > 0 && xAxis.length == series.length && (
          <LineChart
            ...
            series={[{ curve: 'linear', data: series }]}
            width={900}
            height={300}
          />
        )}
