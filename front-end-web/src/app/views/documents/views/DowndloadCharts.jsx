...
export default function DownloadCharts() {
  ...
  const getDownloadedYear = async () => {
    setDefault();
    const response = await axios.get(
      `${process.env.REACT_APP_API_URL}/admin/dashboard/transaction/getDownloadedYear`
    );
    console.log(response.data);
    const list = response.data;
    let xAxisList = [];
  ...
  const getDownloadedMonth = async () => {
    setDefault();
    const response = await axios.get(
      `${process.env.REACT_APP_API_URL}/admin/dashboard/transaction/getDownloadedMonth/${year}`
    );
    console.log(response.data);
    const list = response.data;
    let xAxisList = [];
  ...
  const getDownloadedDay = async () => {
    setDefault();
    const response = await axios.get(
      `${process.env.REACT_APP_API_URL}/admin/dashboard/transaction/getDownloadedDay/${year}/${month}`
    );
    console.log(response.data);
    const list = response.data;
    let xAxisList = [];
  ...
  return (
    <Box sx={{ padding: 3, display: 'flex' }}>
      <Box sx={{}}>
        <Typography sx={{ fontSize: 18, fontWeight: 'bold' }}>Lượt tải</Typography>
