...
export default function ViewCharts() {
  ...
  const getViewsYear = async () => {
    setDefault();
    const response = await axios.get(
      `${process.env.REACT_APP_API_URL}/admin/dashboard/document/getViewsYear`
    );
    console.log(response.data);
    const list = response.data;
    let xAxisList = [];
  ...
  const getViewsMonth = async () => {
    setDefault();
    const response = await axios.get(
      `${process.env.REACT_APP_API_URL}/admin/dashboard/document/getViewsMonth/${year}`
    );
    console.log(response.data);
    const list = response.data;
    let xAxisList = [];
  ...
  const getViewsDay = async () => {
    setDefault();
    const response = await axios.get(
      `${process.env.REACT_APP_API_URL}/admin/dashboard/document/getViewsDay/${year}/${month}`
    );
    console.log(response.data);
    const list = response.data;
    let xAxisList = [];
  ...
  return (
    <Box sx={{ padding: 3, display: 'flex' }}>
      <Box sx={{}}>
        <Typography sx={{ fontSize: 18, fontWeight: 'bold' }}>Lượt xem</Typography>
