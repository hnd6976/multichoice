...
export default function TopDownloaded() {
  ...
  const [topDownloadedList, setTopDownloadedList] = useState([]);
  const getTopDownloadedsAll = async () => {
    const response = await axios.get(
      `${process.env.REACT_APP_API_URL}/admin/dashboard/transaction/getTopDownloadedAll`
    );
    const list = response.data;
    let listTopDownloaded = [];
    list.map((item, index) => {
      let it = { name: '', value: 0 };
      it.name = item[0];
      it.value = item[1];
      listTopDownloaded[index] = it;
    });
    setTopDownloadedList(listTopDownloaded);
  };
  useEffect(() => {
    getTopDownloadedsAll();
  }, []);
  const getTopDownloadedYear = async () => {
    const response = await axios.get(
      `${process.env.REACT_APP_API_URL}/admin/dashboard/transaction/getTopDownloadedYear/${year}`
    );
    const list = response.data;
    let listTopDownloaded = [];
    list.map((item, index) => {
  ...
  const getTopDownloadedMonth = async () => {
    const response = await axios.get(
      `${process.env.REACT_APP_API_URL}/admin/dashboard/transaction/getTopDownloadedMonth/${year}/${month}`
    );
    const list = response.data;
    let listTopDownloaded = [];
    list.map((item, index) => {
  ...
  const getTopDownloadedDay = async () => {
    const response = await axios.get(
      `${process.env.REACT_APP_API_URL}/admin/dashboard/transaction/getTopDownloadedDay/${year}/${month}/${day}`
    );
    const list = response.data;
    let listTopDownloaded = [];
    list.map((item, index) => {
  ...
  useEffect(() => {
    ...
    if (filter == 'year') {
      getTopDownloadedYear();
    }
    if (filter == 'month') {
      getTopDownloadedMonth();
    }
    if (filter == 'day') {
      getTopDownloadedDay();
    }
  ...
  return (
    <Box sx={{ padding: 2, display: 'flex' }}>
      <Box sx={{}}>
        <Typography sx={{ fontSize: 18, fontWeight: 'bold' }}>Top 5 lượt tải</Typography>
            ...
            <Table sx={{ width: 450 }} aria-label="simple table">
              <TableHead>
                <TableRow>
                  ...
                  <TableCell align="center">Lượt tải</TableCell>
                </TableRow>
              </TableHead>
              <TableBody>
                {topDownloadedList.map((row, index) => (
                  <TableRow
                    key={row.name}
